...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'after_name'       => $query->row['after_name'],
				'mini_desc'        => $query->row['mini_desc'],
				'composition'      => $query->row['composition'],
				'information_id'   => $query->row['information_id'],
				'product_hit'      => $query->row['product_hit'],
				'product_new'      => $query->row['product_new']
			);
		} else {
			return false;
		}
	...
	public function getProducts($data = array()) {
        ...
        if ($this->customer->isLogged()) {
            ...
            $results_column = array_column($results, 'product_id');
            unset($results);
            foreach ($query->rows as $result) {
                if (isset($this->session->data['wishlist']) && in_array($result['product_id'], $results_column)) {
                    $wishlist_status = '1';
                } else {
                    $wishlist_status = false;
                }
                $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
                $product_data[$result['product_id']] += ['wishlist_status' => $wishlist_status];
            }
        }else{
            foreach ($query->rows as $result) {
...
//		}
	...
	public function getProductOptions($product_id) {
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			//add customer group support
			$customer_id = $this->customer->getGroupId();
			$customer_id = ($customer_id == NULL)?'1':$customer_id;
            $customer_stuff = "' AND pov.customer_group_id = '" . (int)$customer_id;

			$product_option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_id = '" . (int)$product_id . "' AND pov.product_option_id = '" . (int)$product_option['product_option_id'] . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . $customer_stuff . "' ORDER BY ov.sort_order");
	...
	public function getProductRelated($product_id) {
        ...
        if ($this->customer->isLogged()) {
            ...
            foreach ($query->rows as $result) {
                ...
                $product_data[$result['related_id']] += ['wishlist_status' => $wishlist_status];
            }
        }else{
            foreach ($query->rows as $result) {
                if (isset($this->session->data['wishlist']) && in_array($result['product_id'], $this->session->data['wishlist'])) {
